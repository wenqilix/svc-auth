...
internal class ApiControllerTests {
    ...
    fun `refresh - should return new token when valid token is provided`() {
        ...
        mockMvc.perform(
            get("/sp/refresh").header(ApiController.HEADER_STRING, "Bearer $validToken")
    ...
    fun `refresh - should return 401 unauthorized when invalid token provided`() {
        ...
        mockMvc.perform(
            get("/sp/refresh").header(ApiController.HEADER_STRING, "Bearer $invalidToken")
    ...
    fun `refresh - should return 400 bad request when error occured`() {
        ...
        mockMvc.perform(
            get("/sp/refresh").header(ApiController.HEADER_STRING, "Bearer $errorToken")
    ...
    fun `token - should return token when valid authCode provided`() {
        ...
        mockMvc.perform(
            get("/sp/token").param("authCode", validAuthCode)
    ...
    fun `token - should return 400 bad request when invalid authCode provided`() {
        ...
        mockMvc.perform(
            get("/sp/token").param("authCode", invalidAuthCode)
        ).andExpect(status().isBadRequest)
    }
}
